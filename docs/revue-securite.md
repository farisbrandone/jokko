...
| Commandes acheteur (inc. 42) | ✅ | Accès acheteur par jeton aléatoire haché (SHA-256) ; `returnUrl` de paiement contraint au domaine de la vitrine ; montants figés à la commande (prix relu côté serveur) ; webhook `orders` sous le même secret `verif-hash` que la facturation (503 sans secret, 403 signature invalide, comparaison à temps constant). PII acheteur (nom, téléphone, e-mail) dans `orders` — couverte par la purge/RGPD à ajouter aux fenêtres de rétention. |
