...
- **4 — Vitrine** ✅ : `packages/ui` (tokens + helpers) ; `apps/storefront` Next.js 15 (RSC/ISR) — middleware de résolution du tenant par sous-domaine, accueil/catégorie/fiche produit (OG + JSON-LD)/recherche, instant search via BFF, deep links WhatsApp/SMS/appel. Reste : PWA, i18n, thème éditable, Storybook, `dashboard`/`admin`.
